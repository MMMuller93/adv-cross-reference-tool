...
const { NAME_STOPWORDS: NAME_STOPWORDS_BASE } = require('./adv_lookup');
...
// Pre-stem the stopword set to handle the asymmetry where input tokens get
// stemmed for plural-handling but the base stopword list only contains certain
// forms. Without this, "series" (in NAME_STOPWORDS) stems to "serie" which is
// NOT in the set, so the stemmed manager-side token leaks as "distinctive."
// Stem rule matches distinctiveTokens below: strip trailing 's' only on ≥6 chars.
const NAME_STOPWORDS = new Set(NAME_STOPWORDS_BASE);
for (const w of NAME_STOPWORDS_BASE) {
  if (w.length >= 6 && w.endsWith('s')) NAME_STOPWORDS.add(w.slice(0, -1));
}
