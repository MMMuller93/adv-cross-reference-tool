...
async function checkAdvDatabase(advDb, managerName, opts = {}) {
  ...
  // Strategy 1: exact base-name ilike
  if (baseName && baseName.length >= 3) {
    ...
    if (exact && exact.length > 0) {
      return {
        ...
        registration_type: exact[0].registration_type,
      };
    }
  }

  // Strategy 2: first-word prefix (min 3 chars, per A0.3 evidence — 214 real
  // advisers in ADV have 3-4 char first words; ≥5 would break Patricof-style cases)
  const firstWord = baseName.split(' ')[0];
  if (firstWord && firstWord.length >= 3 && !NAME_STOPWORDS.has(firstWord.toLowerCase())) {
  ...
  // For each Form D related person with a management role, look for matching
  // owners in adviser_owners (owner_type='I' individuals only, with management
  // title) and resolve back to advisers_enriched via firm_crd.
  const persons = parseRelatedPersons(relatedNames, relatedRoles);
  ...
  // Strategy 4 (legacy): last-name on adviser_name with first-name or base-name validation
  if (relatedNames) {
