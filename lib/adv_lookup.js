...
async function checkAdvDatabase(advDb, managerName, opts = {}) {
  ...
  // Strategy 1: substring ilike with TOKEN-SUBSET post-filter.
  //
  // Production bug 2026-05-11: previously this was naive substring ilike
  // `%baseName%`, which produced wrong matches via embedded substrings:
  //   - baseName "KIG" → ilike '%KIG%' matches IKIGAI (kig embedded in ikigai)
  //   - baseName "Canyon Capital" → ilike '%Canyon Capital%' matches "East Canyon Capital"
  // ilike doesn't have a word-boundary operator. Word-boundary regex was
  // attempted but failed on punctuation cases (e.g., "Patricof Co.\b" doesn't
  // match "PATRICOF CO. LLC" because `.` followed by space has no word break).
  // Token-subset is the cleaner test: tokenize both names (alphanumeric only,
  // lowercased), require every alphabetic token of baseName (≥2 chars) to
  // appear as a full token in the adviser_name.
  const toTokens = (s) => (s || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').split(/\s+/).filter(t => t.length >= 2);
  if (!personOnly && baseIsMeaningful) {
    const { data: candidates } = await advDb
    ...
    if (candidates && candidates.length > 0) {
      const baseToks = toTokens(baseName).filter(t => /[a-z]/.test(t));
      const filtered = candidates.filter(c => {
        const advToks = new Set(toTokens(c.adviser_name));
        return baseToks.every(t => advToks.has(t));
      });
      if (filtered.length > 0) {
        // Pick the SHORTEST match — most likely the bare firm name
        const best = filtered.slice().sort((a, b) => (a.adviser_name || '').length - (b.adviser_name || '').length)[0];
        return {
          found: true,
    ...
    if (tokens.length >= 2) {
      ...
      if (fwIsStopword && swIsDistinctive) {
        ...
        if (twoHits && twoHits.length > 0) {
          // Token-subset post-filter (same anti-embedded-substring guard as Strategy 1)
          const baseToks = toTokens(twoWord).filter(t => /[a-z]/.test(t));
          const filtered = twoHits.filter(c => {
            const advToks = new Set(toTokens(c.adviser_name));
            return baseToks.every(t => advToks.has(t));
          });
          if (filtered.length > 0) {
  ...
  // Strategy 2 (first-word prefix `${firstWord}%`) — REMOVED 2026-05-11.
  //
  // Live evidence from production sample of 15 with-ADV matches: this strategy
  // produced multiple wrong-firm matches via loose first-word prefix:
  //   - "Material Ventures, LP" → MATERIAL LLC (different firm)
  //   - "Front Porch Venture Partners SPV, LP" → FRONT FINANCIAL INC
  //   - "Moreno VC LLC" → MORENO CAPITAL (after VC LLC was wrongly state-stripped)
  //   - "MVP II Co-Invest LLC" → MVP MANAGEMENT GMBH (German entity)
  //   - "Backbone NC LLC" → BACKBONE PLANNING PARTNERS LLC (different firm)
  //
  // Every legitimate case Strategy 2 caught (KIG GP/LLC → KIG INVESTMENT MGMT,
  // HighVista GP → HIGHVISTA STRATEGIES, Canyon Capital Advisors → CANYON
  // CAPITAL ADVISORS, Patricof Co. Master → PATRICOF CO., etc.) is already
  // caught by Strategy 1 substring `%baseName%` because the manager's base name
  // appears IN the adviser's name. Strategy 2 was net-negative.
  //
  // Strategy 1.5 (two-word substring for stopword-first compound names) still
  // covers the Capital Factory class of cases.
...
/**
 * searchAdvByName — variant-laddered wrapper around checkAdvDatabase.
 *
 * `checkAdvDatabase` is a per-name primitive: pass it ONE name string, get
 * back whether that exact name matches an adviser. It can't handle the
 * Hash3/Ineffable class where the Form D series-master has a tail token
 * ("Series", "Master", "Opportunity") that's not in the SEC adviser_name.
 *
 * This wrapper:
 *   1. Generates name variants via lib/name_variants#generateVariants
 *      (full → strip strategy tail → first 2 tokens → first distinctive token)
 *   2. Tries each variant against checkAdvDatabase in order
 *   3. Applies passesStricterCrdGate from lib/name_matcher to reject
 *      acronym/short-token false positives (TMS Angels → TMS Capital Mgmt class)
 *   4. Returns the first variant whose match passes the gate, OR
 *      `{found: false, variants_tried, gate_rejections}` if none do.
 *
 * Same code path that fixes Hash3/Ineffable in the enrichment engine. This
 * wrapper unifies it for callers that previously called checkAdvDatabase
 * with the raw name (e.g., detect_compliance_issues.js).
 *
 * @param {SupabaseClient} advDb
 * @param {string} rawName - Form D series-master (or similar input)
 * @param {object} [opts]
 * @param {string} [opts.relatedNames] - Form D related_names (pipe-separated, used for acronym corroboration)
 * @param {string} [opts.relatedRoles] - Form D related_roles
 * @param {boolean} [opts.personOnly]  - forwarded to checkAdvDatabase (for A8 timing-lag flow)
 * @returns {Promise<{found, source, crd, adviser_name, registration_type, matched_variant?, gate?, variants_tried?, gate_rejections?}>}
 */
async function searchAdvByName(advDb, rawName, opts = {}) {
  const { generateVariants } = require('./name_variants');
  const { passesStricterCrdGate } = require('./name_matcher');

  const variants = generateVariants(rawName, { extraStripper: extractBaseName });
  // Always try the raw input first too in case it's already canonical
  const tries = Array.from(new Set([rawName, ...variants].filter(v => v && v.length >= 3)));
  ...
  for (const variant of tries) {
    const hit = await checkAdvDatabase(advDb, variant, {
      ...
      personOnly: opts.personOnly || false,
    });
    ...
    if (!gate.pass) {
      gateRejections.push({ crd: hit.crd, variant, adviser_name: hit.adviser_name, reason: gate.reason });
      continue;
    }
    return { ...hit, matched_variant: variant, gate };
  }

  // Opt-in IAPD live fallback. Caller sets opts.iapdFallback=true to use it.
  // Catches firms registered AFTER our local cache cron refreshed. The local
  // cache is the source of truth for most callers (fast, deterministic);
  // IAPD is the safety net.
  if (opts.iapdFallback) {
    const { iapdLiveLookupVariants } = require('./iapd_live');
    const iapdRes = await iapdLiveLookupVariants(tries);
    if (iapdRes.found) {
      // Run the same stricter gate on the live result so we don't accept
      // acronym FPs from IAPD any more than from the local cache.
      ...
      gateRejections.push({
        crd: iapdRes.crd,
        variant: iapdRes.matched_variant,
        adviser_name: iapdRes.adviser_name,
        reason: `iapd_${gate.reason}`,
      });
    }
  }
  ...
  return {
    found: false,
    variants_tried: tries,
    gate_rejections: gateRejections.length ? gateRejections : undefined,
  };
}
